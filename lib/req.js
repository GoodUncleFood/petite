...
request.process = function(req, res) {
  ...
  tmp = request.checkCors(app.config.cors, data);
  ...
  if (typeof(tmp) == 'number') {
    console.log('Check Header error:', data);
    app.lib.res.send(req, res, tmp);
    return false;
  }
...
// Check for an OPTIONS request (if CORS is enabled)
request.checkCors = function(cors, data) {
  cors = typeof(cors) == 'boolean' && cors ? true : false;
  data = typeof(data) == 'object' ? data : {};
  if (cors) {
    if (typeof(data.method) == 'string' && data.method == 'options') {
      return 200;
    } else {
      return;
    }
